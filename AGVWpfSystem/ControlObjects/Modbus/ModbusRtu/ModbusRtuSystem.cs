...
namespace ControlObjects.Modbus.ModbusRtu {
    class ModbusRtuSystem : DeviceBase {
        ModbusTcpServer tcpServer;
        ModbusTcpNet modusRtu;
        public EventHandler ConnectFail_Happened;
        private bool connectFlag;

        public ModbusRtuSystem(string name, string com, int baudRate,string ip) : base(name) {
           // modusRtu = new ModbusTcpNet(ipAddress, port, station);
            tcpServer = new ModbusTcpServer();
            tcpServer.StartModbusRtu(com,baudRate);
        }
        ...
        protected  ushort ReadUInt16(DeviceAdress adress) {
            ...
            ushort value = modusRtu.ReadUInt16(modbusAdrss.SignalAdress).Content;
            return value;
        }

        protected  uint ReadUInt32(DeviceAdress adress) {
            ModbusAdress modbusAdrss = (ModbusAdress)adress;
            uint value = modusRtu.ReadUInt32(modbusAdrss.SignalAdress).Content;
            return value;
        }

        protected  ulong ReadUInt64(DeviceAdress adress) {
            ModbusAdress modbusAdrss = (ModbusAdress)adress;
            ulong value = modusRtu.ReadUInt64(modbusAdrss.SignalAdress).Content;
            return value;
        }
        ...
        protected  byte ReadByte(DeviceAdress adress) {
            ModbusAdress modbusAdrss = (ModbusAdress)adress;
            byte value = Convert.ToByte(modusRtu.ReadUInt16(modbusAdrss.SignalAdress).Content >> 8);
            return value;
        }
        ...
        protected override void ReadRealTimeSignal() {
            ...
            while (connectFlag && inputThreadRun) {
                OperateResult<byte[]> results = modusRtu.Read(((ModbusAdress)startInputsAdress).SignalAdress, (ushort)SynInputs.Length);
        ...
        protected override void WriteRealTimeSignal() {
            ...
            while (connectFlag && outputThreadRun) {
                modusRtu.Write(((ModbusAdress)startOutputsAdress).SignalAdress, SynOutputs);
                Thread.Sleep(100);
            }
        }
